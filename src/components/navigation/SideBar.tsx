...
const SideBar = ({
  ...
  return (
    <section className={` h-screen border-r border-gray-200  ${isOpen.small ? "fixed inset-0 bg-black/60 flex z-30 " : ` md:inline-block hidden  transition-all duration-500 ${isOpen.large ? "w-24" : "w-60"}`}  transition-all duration-500   cursor-pointer`}>
      <section className={`cursor-default h-full bg-white shadow-md overflow-clip   ${isOpen.small ? "w-full sm:w-64" : ""}`}>
        ...
        <div className={`w-full h-[calc(100vh-6rem)] mt-2 ${isOpen.large ? "p-4" : "  p-2 pt-4 pl-4 "}  overflow-y-auto  [&::-webkit-scrollbar]:hidden `}>
          {sidebarData.map((sideData) => {
            ...
            return (
              <NavLink key={sideData.path} to={`${sideData.path}`} className={({ isActive }) => ` relative group rounded-md border-l-4 transition-all duration-500  flex font-medium items-center
                    ${isOpen.large
      ...
      <button onClick={onToggleSidebarSmall} className={`absolute top-6 right-4 z-50 p-1 bg-white rounded-md text-black hover:text-rose-400 ${isOpen.small ? "" : "hidden"}`}>
