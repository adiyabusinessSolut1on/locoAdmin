  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isExternal) {
        ...
        formData.append("link", value?.image)
      }
      formData.append("donwloadable", value?.donwloadable)
      formData.append("isExternal", isExternal)

      const response = await updatePost({
        data: formData,
        method: isUpdate && !isError ? "PUT" : "POST",
        path: isUpdate && !isError ? `/important_link/${id}` : `/important_link/create`,
      }).unwrap()
      ...
      <form className="w-full h-[calc(100vh-6rem)] overflow-hidden   rounded-md" onSubmit={handleCreate}>
        <div className="flex-1 h-full p-6 rounded font-montserrat">
          <div className="flex pb-2">
            <h2 className="md:text-4xl text-[28px] font-bold text-gray-500 font-mavenPro">Creat Document Form</h2>
            ...
            <div className="grid items-center grid-cols-1 gap-4 py-4 md:grid-cols-2">
              ...
              {isExternal ? (
                <div className="flex flex-col w-full gap-1">
                  <input value={value?.link && value?.link} onChange={(e) => OnchangeValue("link", e.target.value)} type="text" placeholder="Enter Url" className="w-full h-10 pl-4 font-medium bg-blue-100 border border-transparent rounded-md outline-none focus:border-blue-200 " />
                </div>
              ) : (
              ...
              <div className="flex items-center order-2 w-full gap-2 md:order-1 ">
                <label htmlFor="" className="mb-1 font-medium text-gray-500">Download Able</label>
                <input checked={value?.donwloadable} onChange={() => OnchangeValue("donwloadable", !value?.donwloadable)} className="  rounded-[7px] outline-none border border-transparent bg-blue-100 focus:border-blue-200" type="checkbox" />
              </div>
              ...
              <div className="flex order-1 w-full gap-2 md:order-2">
                ...
                <input checked={isExternal} onChange={() => { setIsExternal(!isExternal); setValue({ ...value, link: '', image: '' }); setImagePreview(null) }} className="  rounded-[7px] outline-none border border-transparent bg-blue-100 focus:border-blue-200" type="checkbox" />
              </div>
            </div>
