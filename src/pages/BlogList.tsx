...
const BlogList = () => {
  ...
  const filteredBlogs = response?.data?.filter((blog: BlogSTyepes) =>
  ...
  const handleConfirmDelete = () => {
    ...
    deletPost({ url: `/blog/delete-blog/${isModalOpen.id}` }).then((res) => {
      if (res.data.success) {
        toast.dismiss();
          ...
          <section className="w-full overflow-auto border-2 [&::-webkit-scrollbar]:hidden rounded-lg border-gray-200 shadow-md bg-white">
            ...
            <div className="h-[380px] overflow-y-auto [&::-webkit-scrollbar]:hidden min-w-[900px] bg-gray-50">
              {isLoading ? (
                ...
                currentBlogs?.map((blog: BlogSTyepes, i: number) => (
                  <section
                    key={i}
