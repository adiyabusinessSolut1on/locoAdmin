...
const UserList = () => {
  ...
  const filteredData = data?.filter((item: UserTypes) =>
      ...
      <section className={`  md:pl-0 p-4 h-full  w-full rounded-md   mx-auto [&::-webkit-scrollbar]:hidden `}>
        <section className={`md:p-8 p-6 h-full border-gray-200 rounded-md  max-w-full w-full `}>
          <div className="flex items-center mb-2 md:mb-6">
            <h1 className=" text-[28px] font-bold md:text-4xl text-gray-600 font-mavenPro">Users</h1>
          </div>
          ...
          <div className="flex justify-between mb-4">
            ...
            <div className="relative flex items-center self-end ">
              <button className={` px-2 py-1 bg-[#1f3c88] hover:bg-[#2d56bb]  text-[#DEE1E2] font-semibold rounded shadow-xl md:px-4 md:py-2  sm:self-center`} onClick={creatUserhandler}>
                ...
                currentUsers?.map((user: UserTypes, i: number) => (
                  <section key={i} className="grid items-center gap-6 py-2 pl-6 pr-4 border-t-2 border-gray-200 grid-cols-customUsers group hover:bg-gray-50">
                    ...
                    <div className="flex items-center justify-center">
                      {user?.image ? (
                        <img src={getMediaUrl(user?.image, "userProfile")} alt="user Image" className="object-cover w-24 h-24 rounded-full " />
                      ) : (
                        <span className="text-sm font-bold text-gray-400">No Image</span>
                      )}
                    </div>
                    <span className={`  font-semibold text-center  rounded-full  `}>{user?.name ? user?.name : "---"}</span>
                    <span className={`  font-semibold text-center  rounded-full  `}>{user?.email ? user?.email : "--"}</span>
                    <span className={`  font-semibold text-center  rounded-full  `}>{user?.mobile ? user?.mobile : "------"}</span>

                    <div className="grid justify-center gap-2">
                      <button className="px-3 py-2 text-sm font-semibold text-white rounded-md bg-[#1f3c88] hover:bg-[#2d56bb]" onClick={() => updateuser(user)}>
