      ...
      <section className={`  md:pl-0 p-4 h-full  w-full rounded-md   mx-auto [&::-webkit-scrollbar]:hidden `}>
        <section className={` md:p-8 p-6 h-full border-gray-200 rounded-md  max-w-full w-full `}>
          <div className="flex items-center mb-2 md:mb-6">
            <h1 className=" text-[28px] font-bold md:text-4xl text-gray-600 font-mavenPro">Companies</h1>
          </div>
            ...
            <div className="relative flex items-center self-end ">
              <button className={` px-2 py-1 bg-[#1f3c88] hover:bg-[#2d56bb]  text-[#DEE1E2] font-semibold rounded shadow-xl md:px-4 md:py-2  sm:self-center`}>
                <Link to={"/sponsor/company_form"}>
                  <span className="hidden md:inline-block">Add Sponsor Company</span>
              ...
              {isLoading ? (
                ...
                currentSponser?.map(
                  (company: SponsorCompanytypes, i: number) => (
                    <section key={i} className="grid items-center gap-6 py-2 pl-6 pr-4 border-t-2 border-gray-200 grid-cols-customCompanies group hover:bg-gray-50">
                      <span>{i + 1}</span>
                      <span className={`  font-semibold text-center  rounded-full  `}>{company?.name}</span>

                      <span className="text-sm font-semibold break-words break-all text-ellipsis">{company?.type}</span>

                      <div className="flex items-center justify-center">
                        {company?.image ? (
                          <img src={getMediaUrl(company?.image, "sponsorImage")} alt="company Logo" className="object-contain w-full text-sm text-center rounded-lg h-1/2 " />
                        ) : (
                          <span className="text-sm font-bold text-gray-400">No Image</span>
                        )}
                      </div>
                      <span className="flex justify-center ml-2 text-sm font-semibold cursor-pointer hover:underline hover:text-sky-400" typeof="button" onClick={() => handlingVideo(getMediaUrl(company?.video, "sponsorVideo"))}>
                        {getMediaUrl(company?.video, "sponsorVideo") ? "View Video" : "--"}
                      </span>

                      <div className={`line-clamp-5 `} dangerouslySetInnerHTML={{ __html: company?.description, }} />
                      <span onClick={() => company?.link && handleLinkClick(company.link)} className={` text-sm font-semibold text-center ${company?.link ? "hover:underline hover:text-sky-400 " : ""} break-words break-all cursor-pointer `}>{company?.link ? "Official site" : "----"}</span>

                      <ConfirmationDialog show={dialogCrendial?.showDialog} onClose={handleCloseDialog} onConfirm={handleConfirmRedirect} />

                      <span className="flex justify-center ml-2 text-sm font-semibold ">{company?.active === true ? "Active" : "not Active"}</span>
                      <span className="flex justify-center ml-2 text-sm font-semibold ">{company?.products.length || 0}</span>
                      <div className="flex justify-center">
                        <button className="px-2 py-2 text-white bg-blue-400 rounded-md hover:bg-blue-500">
                          <Link to={`/sponsor/profile/${company._id}`} className="flex items-center justify-center text-sm font-semibold "><PiEye className="w-4 h-4" /></Link>
                        </button>
                      </div>

                      <div className="grid justify-center gap-2">
                        <button className="px-3 py-2 text-sm font-semibold text-white rounded-md bg-[#1f3c88] hover:bg-[#2d56bb]" onClick={() => updatecompany(company)}>Edit</button>
                        <button className="px-3 py-2 text-sm font-semibold text-white rounded-md bg-rose-600 hover:bg-rose-700" onClick={() => deletcompany(company._id)}>Delete                        </button>
                      </div>
                    </section>
                  )
                )
          ...
          <Pagination<SponsorCompanytypes> currentPage={currentPage} apiData={filteredData} itemsPerPage={itemsPerPage} handleClick={handleClick} />
        </section>
      </section>
    </>
  );
