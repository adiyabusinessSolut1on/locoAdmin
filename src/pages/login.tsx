...
const Login = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    createPost({ data: { ...logindata }, path: "/login", }).then((res) => {
      console.log("res: ", res);
      if (res?.data?.success) {
        localStorage.setItem("user", res?.data?.token);
        dispatch(setUserToken(res?.data?.token));
        toast.success("You have been Logged in", { autoClose: 3000, });
        setTimeout(() => {
          navigate("/");
        }, 2000);
      } else {
  ...
  return (
    <React.Fragment>
      ...
      <div className="flex flex-col items-center justify-center h-screen px-4 mx-auto  w-[100%]  sm:w-[80%] md:w-[50%]  lg:w-[40%]">
        <div className="">
          <h4 className="text-[24px] md:text-3xl lg:text-4xl font-bold pb-2 text-gray-600">Log in to your Account</h4>
          <p className="text-sm font-medium text-gray-600 sm:text-base">Welcome back! Please Login to continue</p>
        </div>

        <form className="flex flex-col items-center justify-center w-full py-4 lg:p-8" onSubmit={handleSubmit}>
          <div className="relative w-full">
            ...
            <input
              ...
              onChange={handleChange} value={logindata.email} name="email" type="email" required placeholder="Email"
            />
          </div>
          ...
          <div className="relative w-full mb-4 lg:mb-6">
            ...
            <button className="absolute text-gray-400 right-4 top-[10px]" role="button" onClick={(e) => { e.preventDefault(); setVisible((prev) => !prev); }}>
              {isVisible ? (<FiEye className="w-5 h-5 text-blue-400" />) : (<FiEyeOff className="w-5 h-5 " />)}
            </button>
          </div>
          ...
          <div className="grid w-full pb-2">
            ...
            <button type="submit" className="px-4 py-2 text-white bg-[#1F3C88] border rounded-md disabled:bg-gray-600 hover:bg-[#1f5e88]">Login</button>
          </div>
